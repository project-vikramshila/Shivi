...
exports.runFailureRecoveryTests = runFailureRecoveryTests;
const testHelper_1 = require("../utils/testHelper");
const mocks_1 = require("../fixtures/mocks");
const runner = new testHelper_1.TestRunner('Failure & Recovery');
async function runFailureRecoveryTests() {
    ...
    await runner.test('Failure: Gemini offline triggers local AI fallback', async () => {
        ...
        try {
            ...
            fallbackUsed = true;
            ai = mocks_1.mockLocalAIService;
        }
        testHelper_1.Assert.isTrue(fallbackUsed);
        testHelper_1.Assert.isDefined(ai);
        const response = await ai.generateResponse('Test prompt');
        testHelper_1.Assert.isString(response);
        testHelper_1.Assert.includes(response, 'Local:');
    });
    ...
    await runner.test('Failure: Database offline switches to local memory', async () => {
        ...
        testHelper_1.Assert.isTrue(useLocalStorage);
    });
    ...
    await runner.test('Failure: IPC handler crash logged, app continues', async () => {
        ...
        testHelper_1.Assert.ok(errorLog.length > 0);
        testHelper_1.Assert.includes(errorLog[0].message, 'IPC');
    });
    ...
    await runner.test('Failure: OCR fails, user prompted for input', async () => {
        ...
        testHelper_1.Assert.isTrue(ocrFailed);
        testHelper_1.Assert.isTrue(fallbackActivated);
    });
    ...
    await runner.test('Failure: Automation interrupted, resume from checkpoint', async () => {
        ...
        testHelper_1.Assert.isTrue(resumed);
        ...
        const executeWithRetry = async () => {
            while (attempt < maxRetries) {
                ...
                try {
                    ...
                    await (0, testHelper_1.sleep)(10); // Simulated wait
                }
            }
            return false;
        };
        const success = await executeWithRetry();
        testHelper_1.Assert.ok(!success || retries.length >= 0);
        testHelper_1.Assert.ok(attempt <= maxRetries);
    });
    ...
    await runner.test('Failure: Permission denied gracefully handles', async () => {
        ...
        testHelper_1.Assert.isTrue(permissionDenied);
        testHelper_1.Assert.isTrue(userNotified);
    });
    ...
    await runner.test('Failure: Task fails, auto-retry with same strategy', async () => {
        ...
        testHelper_1.Assert.isTrue(finalSuccess);
        testHelper_1.Assert.equal(failAttempt, 2);
    });
    ...
    await runner.test('Failure: Long task interrupted, pause and resume', async () => {
        ...
        testHelper_1.Assert.equal(paused.status, 'paused');
        // Resume
        const resumed = {
            ...paused,
            status: 'running',
    ...
    await runner.test('Failure: Critical service down, warn user', async () => {
        ...
        testHelper_1.Assert.ok(downServices.length === 1);
        testHelper_1.Assert.includes(downServices[0], 'database');
    });
    ...
    await runner.test('Failure: Memory usage spike, trigger cleanup', async () => {
        ...
        testHelper_1.Assert.ok(memoryUsage <= cleanupThreshold);
    });
    ...
    await runner.test('Failure: Autonomous workflow error triggers AI decision', async () => {
        ...
        testHelper_1.Assert.isTrue(errorOccurred);
        testHelper_1.Assert.isTrue(aiDecisionMade);
    });
    ...
    await runner.test('Failure: Multiple failures trigger fallback chain', async () => {
        ...
        testHelper_1.Assert.equal(strategies.length, 3);
        testHelper_1.Assert.includes(strategies[2], 'ask-user');
    });
    ...
    await runner.test('Recovery: User receives Hindi narrative during recovery', async () => {
        ...
        Object.entries(narratives).forEach(([scenario, message]) => {
            testHelper_1.Assert.isString(message);
            testHelper_1.Assert.ok(message.length > 0);
        });
    });
    ...
    await runner.test('Recovery: System fully recovers after major failure', async () => {
        ...
        testHelper_1.Assert.isTrue(recovered);
        testHelper_1.Assert.equal(systemState.after, 'operational');
    });
    runner.print();
}
